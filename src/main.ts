...
async function fetchProducts() {
  try {
    ...
    const selected = allProducts.filter(p => p.blocks && p.blocks.includes("Selected Products"));
    renderProducts(selected, 'product-grid', 'Add To Cart');

    const arrivals = allProducts.filter(p => p.blocks && p.blocks.includes("New Products Arrival"));
    ...
    if (catalogContainer) {
      ...
      const renderCatalog = () => {
        ...
        renderPagination(Math.ceil(filteredProducts.length / itemsPerPage), currentPage);
      ...
      // ЛОГІКА ФІЛЬТРІВ
      const applyFilters = () => {
        const category = (document.getElementById('filter-category') as HTMLSelectElement)?.value || 'all';
        const color = (document.getElementById('filter-color') as HTMLSelectElement)?.value || 'all';
        const size = (document.getElementById('filter-size') as HTMLSelectElement)?.value || 'all';
        const sale = (document.getElementById('filter-sale') as HTMLInputElement)?.checked || false;
        const sort = (document.getElementById('sort-select') as HTMLSelectElement)?.value || 'default';
        ...
        filteredProducts = allProducts.filter(p => {
          ...
          if (sale && p.salesStatus !== true && p.salesStatus !== "true") return false;
          return true;
        });
        ...
        else if (sort === 'popularity') filteredProducts.sort((a, b) => (b.popularity || 0) - (a.popularity || 0));
      ...
      document.getElementById('reset-filters')?.addEventListener('click', () => {
        if (document.getElementById('filter-category')) (document.getElementById('filter-category') as HTMLSelectElement).value = 'all';
        if (document.getElementById('filter-color')) (document.getElementById('filter-color') as HTMLSelectElement).value = 'all';
        if (document.getElementById('filter-size')) (document.getElementById('filter-size') as HTMLSelectElement).value = 'all';
        if (document.getElementById('sort-select')) (document.getElementById('sort-select') as HTMLSelectElement).value = 'default';
        if (document.getElementById('filter-sale')) (document.getElementById('filter-sale') as HTMLInputElement).checked = false;
    ...
    if (topSetsContainer) {
      const topProducts = allProducts.filter(p => p.blocks && p.blocks.includes("Top Best Sets")).slice(0, 3);
      const itemsToShow = topProducts.length > 0 ? topProducts : allProducts.slice(0, 3); 

      topSetsContainer.innerHTML = itemsToShow.map(p => `
...
(window as any).addToCart = (id: string, color: string = '', size: string = '') => {
    ...
    const existingItem = cart.find((item: any) => item.id === id && item.color === color && item.size === size);
