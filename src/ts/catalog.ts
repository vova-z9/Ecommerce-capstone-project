...
function setupListeners() {
  ...
  const applyFilters = () => {
    ...
    const colorElem = document.getElementById(
      "filter-color",
    ) as HTMLSelectElement;
    const sizeElem = document.getElementById(
      "filter-size",
    ) as HTMLSelectElement;
    const saleElem = document.getElementById("filter-sale") as HTMLInputElement;
    const sortElem = document.getElementById(
      "sort-select",
    ) as HTMLSelectElement;
    ...
    filteredProducts = allProducts.filter((p: Product) => {
      ...
      const matchColor =
        color === "all" || itemColor === color.toLowerCase().trim();

      const itemSizes = getSizesArray(p.size || "");
      const selectedSizes = getSizesArray(size);
      const matchSize =
        size === "all" || selectedSizes.some((s) => itemSizes.includes(s));

      const matchSale =
        !saleOnly || p.salesStatus === true || p.salesStatus === "true";
